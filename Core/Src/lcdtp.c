...
void LCD_DrawString ( uint16_t usC, uint16_t usP, const char * pStr )
{
	while ( * pStr != '\0' )
	{
		if ( ( usC - LCD_DispWindow_Start_COLUMN + WIDTH_EN_CHAR ) >   320)
		{
			usC = LCD_DispWindow_Start_COLUMN;
			usP += HEIGHT_EN_CHAR;
		}
		
		if ( ( usP - LCD_DispWindow_Start_PAGE + HEIGHT_EN_CHAR ) > 240)
		{
			usC = LCD_DispWindow_Start_COLUMN;
...
{	
	float rad = 3.14159265359 / 180;

	// Filling each pixel corresponding
	// to every angle from 0 to 360
	for (float theta = 0; theta < 360; theta += 0.3)
	{
		 double x = R * cos(theta * rad);
		 double y = R * sin(theta * rad);
